...
extern const char * scale_output_file;
#define max(x,y) (((x)>(y))?(x):(y))
#define min(x,y) (((x)<(y))?(x):(y))

void output_target(double value, FILE *input);
void output(int index, double value, FILE *input);
...
#define SKIP_ELEMENT\
	...
	while (readline(fp) != NULL)
	{
		...
		SKIP_TARGET
                //printf("line after skiptarget is %s \n",p);
		...
		while (sscanf(p, "%d:%*f", &index) == 1)
		{
			...
			num_nonzeros++;
                ...
                //printf("line after skipelement is %s \n",p);
                //printf("line 3 is %s \n",line);
	}
	...
	while (readline(fp) != NULL)
	{
		...
		SKIP_TARGET
                //printf("line 11 after skipetarget is %s \n",p);
		...
		while (sscanf(p, "%d:%lf", &index, &value) == 2)
		{
			...
			next_index = index + 1;
                ...
                //printf("line 11 after skipeelement is %s \n",line);
	...
	FILE *fp_result = fopen(scale_output_file, "w+");
	if (fp_result == NULL)
	{
	...
	while (readline(fp) != NULL)
	{
		...
		output_target(target,fp_result);
		...
		while (sscanf(p, "%d:%lf", &index, &value) == 2)
		{
			for (i = next_index; i < index; i++)
			{
				output(i, 0, fp_result);
			}
			output(index, value, fp_result);
