...
FILE *fp_train_result = NULL;
void print_train(const char *s)
{
	//fprintf(input, "%s ", s);
	fputs(s, fp_train_result);
	fflush(fp_train_result);
}

void exit_with_train_help()
{
	printf(
...
const char * train_output_file = "/tmp/train.result";
...
int train_main(int argc, char **argv)
{
	...
	fp_train_result = fopen(train_output_file, "w+");

	parse_command_line(argc, argv, input_file_name, model_file_name);
	read_problem(input_file_name);
	error_msg = svm_check_parameter(&prob, &param);
	if (fp_train_result== NULL)
	{
...
{
	int i;
	void (*print_func)(const char*) = print_train;// default printing to stdout
